...
import GUI.javaFxEnvironment.PolygonMesh;
import GUI.javaFxEnvironment.PolygonMeshView;
import GUI.utils.Quaternion;
import GUI.utils.Vector3;
import GUI.utils.Vector4;
...
public class AttitudeFx {
	...
	private Color modelColor = Color.SILVER;
	private Color gridColor = Color.BLACK;
	private Color backgroundColor = Color.color(0.25,0.25,0.25);
	...
	public  void start(JFXPanel fxpanel, String objectFilePath,  double modelScale) {
		...
		final Group grid = createGrid(2000, 150);
		
		Group staticEnvironment = new Group();
		staticEnvironment.getChildren().add(grid);
		
		root.getChildren().add(staticEnvironment);
		root.getChildren().add(camera);
		root.getChildren().add(model);
		
		Scene primaryScene = new Scene(root, 450, 450, true, SceneAntialiasing.BALANCED);
		
		primaryScene.setFill(backgroundColor);
		primaryScene.setCamera(camera);		

		initMouseControl(primaryScene, fxpanel, camera);

		fxpanel.setScene(primaryScene);
	...
	public void modelRotation(Quaternion q) {
		...
		try {
			...
			System.out.println("model rotation failed. "+exp.getStackTrace());
		}
	}
...
private   Group loadModel(String fileString) {
    Group modelRoot = new Group();
	PhongMaterial material = new PhongMaterial();
    material.setDiffuseColor(modelColor);
